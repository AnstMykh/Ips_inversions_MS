#could take a lot of time, so running on the background
### loading packages
if (!require("gplots")) {
...
setwd("~/mut_load/OUTPUT/outliers")
path_to_file <- ""
path_to_output <- ""
...
for (filename in list_of_data_files) {
  ...
  data_filename <- file.path(path_to_file, paste(file_name_no_ext, "ld", sep="."))
  ld_contig <- read.table(data_filename, strip.white = TRUE, header = FALSE, stringsAsFactors=FALSE)
  
  
  positions_filename <- file.path(path_to_file, paste(file_name_no_ext, "pos","txt", sep="."))
  SNP_positions <- read.table(positions_filename, header = TRUE)
  SNP_positions <- as.vector(SNP_positions[, 1])
  
  
  #matrix of windows of 10 kb
  max_kb <- SNP_positions[length(SNP_positions)]
  window_size <- 10000
  window_max <- ceiling(max_kb/window_size) 
  
  column_names <- c()
  row_names <- c()
  ld_matrix <- matrix( 0, nrow = window_max, ncol = window_max)
  ...
  for (window_x in 1:window_max) {
    for (window_y in window_x:window_max) {
      ...
      x_indeces <- which(SNP_positions >= col_x_start & SNP_positions < col_x_end)
      y_indeces <- which(SNP_positions >= col_y_start & SNP_positions < col_y_end)
      ...
      row_names <- append(row_names, paste(col_y_end))
      row_names <- unique(row_names)
      ld_matrix[window_y, window_x] <- R2_2nd
      ld_matrix[window_x, window_y] <- R2_2nd
      
    }
    column_names <- append(column_names, paste(col_x_end))
  }
  
  colnames(ld_matrix) <- c(column_names)
  rownames(ld_matrix) <-c(row_names)
  
  write.table(ld_matrix, file = paste(path_to_output,file_name_no_ext, '.txt'  ,sep = ''))
  
}

## now for creating a matrix where the upper triangle is LD for only homozygous (one of the haplotypes) 
individuals and the lover triange includes all
...
for (filename in list_of_data_files) {
  ...
  #matrix of windows of 10 kb 
  max_kb <- max(SNP_positions_inv[length(SNP_positions_inv)],SNP_positions_contig[length(SNP_positions_contig)]) ## poklady maxymalne znachannya z dwoch 
  window_size <- 10000
  window_max <- ceiling(max_kb/window_size) 
